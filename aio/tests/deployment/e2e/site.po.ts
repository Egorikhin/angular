...
export class SitePage {
  ...
  async enterSearch(query: string) {
    let searchInput = element(by.css('input[type=search]'));
  ...
  async getDocViewerText() {
    let docViewer = element(by.css('aio-doc-viewer'));
    let text = await docViewer.getText();
   ...
   */
  async unregisterSw() {
    let unregisterSwFn = (cb: () => void) => navigator.serviceWorker
        .getRegistrations()
