...
let {relative} = require('canonical-path');
let {generateDocs} = require('./index.js');
let { PROJECT_ROOT, CONTENTS_PATH, API_SOURCE_PATH } = require('../config');

function listener(changeType, fullPath) {
  try {
    let relativePath = relative(PROJECT_ROOT, fullPath);
...
if (process.argv.indexOf('--watch-only') === -1) {
  ...
  let dgeni = new Dgeni([require('../angular.io-package')]);
