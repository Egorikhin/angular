...
<code-example path="ngmodules/src/app/greeting/user.service.ts" region="ctor" header="src/app/greeting/user.service.ts (letructor)" linenums="false">
...
To guard against a lazy loaded module re-importing `GreetingModule`, add the following `GreetingModule` letructor.
...
The letructor tells Angular to inject the `GreetingModule` into itself.
...
and the letructor concludes uneventfully.
...
Now `parentModule` exists and the letructor throws the error.
