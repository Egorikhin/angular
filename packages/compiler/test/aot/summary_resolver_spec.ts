...
import {letantPool} from '@angular/compiler/src/letant_pool';
...

{
  describe('AotSummaryResolver', () => {
    ...
    function serialize(
      ...
      let mockSummaryResolver = new MockSummaryResolver([]);
      let symbolResolver = new StaticSymbolResolver(
    ...
    it('should load serialized summary files', () => {
      let asymbol = symbolCache.get('/a.d.ts', 'a');
    ...
    it('should fill importAs for deep symbols if external symbol re-exports are enabled', () => {
      let libSymbol = symbolCache.get('/lib.d.ts', 'Lib');
      let srcSymbol = symbolCache.get('/src.ts', 'Src');
      init({
        '/src.ngsummary.json':
...
export class MockAotSummaryResolverHost implements AotSummaryResolverHost {
  letructor(private summaries: {[fileName: string]: string}) {}
