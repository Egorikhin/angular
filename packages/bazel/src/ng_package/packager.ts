...
function main(args: string[]): number {
  ...
  let paramFilePath = args[0];

  // Bazel params may be surrounded with quotes
  function unquoteParameter(s: string) { return s.replace(/^'(.*)'$/, '$1'); }

  // Parameters are specified in the file one per line.
  let params = fs.readFileSync(paramFilePath, 'utf-8').split('\n').map(unquoteParameter);
  ...
  let fesm2015 = fesm2015Arg.split(',').filter(s => !!s);
  let fesm5 = fesm5Arg.split(',').filter(s => !!s);
  let esm2015 = esm2015Arg.split(',').filter(s => !!s);
  let esm5 = esm5Arg.split(',').filter(s => !!s);
  let bundles = bundlesArg.split(',').filter(s => !!s);
  let typeDefinitions = typeDefinitionsArg.split(',').filter(s => !!s);
  let srcs = srcsArg.split(',').filter(s => !!s);
  let dataFiles: string[] = dataArg.split(',').filter(s => !!s);
  let modulesManifest = JSON.parse(modulesManifestArg);
  let dtsBundles: string[] = dtsBundleArg.split(',').filter(s => !!s);
  ...
  function writeFileFromInputPath(inputPath: string, fileContent: string) {
    ...
    let outputPath = path.join(out, path.relative(rootDir, inputPath));
  ...
  function writeEsmFile(file: string, suffix: string, outDir: string) {
    ...
    let root = file.substr(0, file.lastIndexOf(`${suffix}/`) + suffix.length + 1);
    let rel = path.dirname(path.relative(path.join(root, srcDir), file));
    if (!rel.startsWith('..')) {
  ...
  Object.keys(modulesManifest).forEach(moduleName => {
    let moduleFiles = modulesManifest[moduleName];
    let relative = path.relative(binDir, moduleFiles['index']);

    moduleFiles['esm5_index'] = path.join(binDir, 'esm5', relative);
    moduleFiles['esm2015_index'] = path.join(binDir, 'esm2015', relative);

    let metadataFile = moduleFiles['metadata'];
    let typingsOutFile = moduleFiles['typings'];
    ...
    if (dtsBundles.length) {
      let metadataContent = rewireMetadata(metadataFile, typingsOutFile);
  ...
  let licenseBanner = licenseFile ? fs.readFileSync(licenseFile, 'utf-8') : '';

  dtsBundles.forEach(bundleFile => {
    let cleanDistPath = bundleFile.replace(dtsBundleFileSuffix, '.d.ts');
  ...
  let rootPackageName = '';
  let packagesWithExistingPackageJson = new Set<string>();
  ...
  Object.keys(modulesManifest).forEach(entryPointPackageName => {
    let entryPointName = entryPointPackageName.substr(rootPackageName.length + 1);
  ...
  function srcDirRelative(from: string, file: string) {
    let result = normalizeSeparators(
  ...
  function copyFile(file: string, baseDir: string, relative = '.') {
    let dir = path.join(baseDir, relative);
    shx.mkdir('-p', dir);
    shx.cp(file, dir);
    ...
    if (file.indexOf('__') >= 0) {
      let outputPath = path.join(dir, ...path.basename(file).split('__'));
      shx.mkdir('-p', path.dirname(outputPath));
  ...
  function amendPackageJson(packageJson: string, parsedPackage: {[key: string]: string}) {
    ...
    let moduleFiles = modulesManifest[packageName];
    if (!moduleFiles) {
  ...
  function getBundleName(packageName: string, dir: string) {
    ...
    let nameParts = packageName.startsWith('@') ? parts.splice(1) : parts;
    let relativePath = Array(nameParts.length - 1).fill('..').join('/') || '.';
    let basename: string;
    if (dir === 'bundles') {
  ...
  function createMetadataReexportFile(
    ...
    let inputPath = path.join(srcDir, `${entryPointName}.metadata.json`);
  ...
  function createTypingsReexportFile(entryPointName: string, license: string, typingsFile: string) {
    let inputPath = path.join(srcDir, `${entryPointName}.d.ts`);
    let content = `${license}
...
`;
  ...
  function createEntryPointPackageJson(dir: string, entryPointPackageName: string) {
    let pkgJson = path.join(srcDir, dir, 'package.json');
    let content = amendPackageJson(pkgJson, {name: entryPointPackageName});
