...
describe('@angular/common ng_package', () => {
  describe('should have the locales files', () => {
    it('/locales', () => {
      let files = shx.ls('locales').stdout.split('\n');
    ...
    it('/locales/extra', () => {
      let files = shx.ls('locales/extra').stdout.split('\n');
      expect(files.some(n => n.endsWith('.d.ts'))).toBe(true, `.d.ts files don't exist`);
      expect(files.some(n => n.endsWith('.js'))).toBe(true, `.js files don't exist`);
    });
  ...
  describe('should have module resolution properties in the package.json file for', () => {
    ...
    it('/', () => {
      let actual = JSON.parse(fs.readFileSync('package.json', {encoding: 'utf-8'}));
      expect(actual['main']).toEqual('./bundles/common.umd.js');
    });
    // https://github.com/angular/common-builds/blob/master/http/package.json
    it('/http', () => {
      let actual = JSON.parse(fs.readFileSync('http/package.json', {encoding: 'utf-8'}));
    ...
    it('/testing', () => {
      let actual = JSON.parse(fs.readFileSync('testing/package.json', {encoding: 'utf-8'}));
    ...
    it('/http/testing', () => {
      let actual = JSON.parse(fs.readFileSync('http/testing/package.json', {encoding: 'utf-8'}));
    ...
    it('/upgrade', () => {
      let actual = JSON.parse(fs.readFileSync('upgrade/package.json', {encoding: 'utf-8'}));
