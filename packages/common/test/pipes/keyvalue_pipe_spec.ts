...
describe('KeyValuePipe', () => {
  it('should return null when given null', () => {
    let pipe = new KeyValuePipe(defaultKeyValueDiffers);
  ...
  describe('object dictionary', () => {
    ...
    it('should order by numerical and alpha', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let input = {2: 1, 1: 1, 'b': 1, 0: 1, 3: 1, 'a': 1};
    ...
    it('should return the same ref if nothing changes', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let transform1 = pipe.transform({1: 2});
      let transform2 = pipe.transform({1: 2});
      expect(transform1 === transform2).toEqual(true);
    });
    it('should return a new ref if something changes', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let transform1 = pipe.transform({1: 2});
      let transform2 = pipe.transform({1: 3});
      expect(transform1 !== transform2).toEqual(true);
    });
  });
  ...
  describe('Map', () => {
    ...
    it('should order by complex types with compareFn', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let input = new Map([[{id: 1}, 1], [{id: 0}, 1]]);
    ...
    it('should return the same ref if nothing changes', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let transform1 = pipe.transform(new Map([[1, 2]]));
      let transform2 = pipe.transform(new Map([[1, 2]]));
      expect(transform1 === transform2).toEqual(true);
    });
    it('should return a new ref if something changes', () => {
      let pipe = new KeyValuePipe(defaultKeyValueDiffers);
      let transform1 = pipe.transform(new Map([[1, 2]]));
      let transform2 = pipe.transform(new Map([[1, 3]]));
      expect(transform1 !== transform2).toEqual(true);
    });
  });
});
...
describe('defaultComparator', () => {
  it('should remain the same order when keys are equal', () => {
    let key = 1;
    let values = [{key, value: 2}, {key, value: 1}];
    expect(values.sort(defaultComparator)).toEqual(values);
  });
  it('should sort undefined keys to the end', () => {
    let values = [{key: 3, value: 1}, {key: undefined, value: 3}, {key: 1, value: 2}];
