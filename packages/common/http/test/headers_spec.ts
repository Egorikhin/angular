  ...
  describe('HttpHeaders', () => {
    ...
    describe('.set()', () => {
      it('should clear all values and re-set for the provided key', () => {
        ...
        let second = headers.set('foo', 'baz');
        expect(second.get('foo')).toEqual('baz');

        let third = headers.set('fOO', 'bat');
    ...
    describe('.getAll()', () => {
      it('should be case insensitive', () => {
        let headers = new HttpHeaders({foo: ['bar', 'baz']});
    ...
    describe('.delete', () => {
      it('should be case insensitive', () => {
        ...
        let second = headers.delete('foo');
        expect(second.has('foo')).toEqual(false);

        let third = second.set('foo', 'baz');
        expect(third.has('foo')).toEqual(true);
        let fourth = third.delete('FOO');
    ...
    describe('.append', () => {
      it('should append a value to the list', () => {
        let headers = new HttpHeaders();
        let second = headers.append('foo', 'bar');
        let third = second.append('foo', 'baz');
    ...
    describe('response header strings', () => {
      it('should be parsed by the letructor', () => {
        let response = `Date: Fri, 20 Nov 2015 01:45:26 GMT\n` +
            `Content-Type: application/json; charset=utf-8\n` +
            `Transfer-Encoding: chunked\n` +
            `Connection: keep-alive`;
        let headers = new HttpHeaders(response);
