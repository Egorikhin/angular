...
module.exports = {
  extract: gulp => done => {
    ...
    let extract = require('./cldr/extract');
    return extract(gulp, done);
  },

  download: gulp => done => {
    let cldrDownloader = require('cldr-data-downloader');
    let cldrDataFolder = path.join(__dirname, 'cldr/cldr-data');
  ...
  closure: gulp => done => {
    let {RELATIVE_I18N_DATA_FOLDER} = require('./cldr/extract');
    ...
    let localeAll = require('./cldr/closure');
