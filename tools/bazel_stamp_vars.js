...
let BUILD_SCM_HASH = _exec(`git rev-parse HEAD`);
...
let LOCAL_CHANGES = _exec(`git status --untracked-files=no --porcelain`) != '';
...
let BUILD_SCM_VERSION = BUILD_SCM_VERSION_RAW.replace(/-([0-9]+)-g/, '+$1.sha-') +
